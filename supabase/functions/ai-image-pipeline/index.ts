...
function resolveModel({ provider, requestedModel, envModel, detail }: ResolveModelParams): ResolvedModel {
  ...
  if (isAutoProvider && !effectiveProvider) {
    // Prefer OpenAI if both keys available (per acceptance criteria), else whichever exists
    if (OPENAI_API_KEY) {
      effectiveProvider = "openai";
      if (DEBUG) {
...
serve(async req => {
  ...
  // X-Supabase-Authorization is used by supabase-js - may include "Bearer " prefix which needs stripping
  ...
  // Extract user token: Priority order for header extraction
  // 1. X-Client-Auth (preferred, raw JWT)
  // 2. X-Supabase-Authorization (supabase-js, may have Bearer prefix - strip it)
  // 3. Authorization header (if it looks like a user JWT starting with "ey")
  let userToken = (xClientAuth ?? "").trim();
  if (!userToken && xSupabaseAuth) {
    // X-Supabase-Authorization may have "Bearer " prefix - use extractBearerToken to handle it
    userToken = extractBearerToken(xSupabaseAuth) ?? "";
  }
  if (!userToken) {
